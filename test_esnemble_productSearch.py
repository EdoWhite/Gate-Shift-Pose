...
parser.add_argument('--max_models', type=int, default=1)
...
def evaluate_combination(rgb_scores_list, depth_scores_list, rgb_paths_list, depth_paths_list, combination_lenght):
    total_scores = []
    total_avg_scores = []
    total_gmean_scores = []
...
combinations_lengths = range(args.min_models, args.max_models+1)
...
for length in combinations_lengths:
    ...
    combined_combinations = list(itertools.combinations(rgb_scores_list+depth_scores_list, length))
    all_comb.extend(combined_combinations)

filtered_combinations = []
for comb in all_comb:
    rgb_count = sum(1 for model in comb if "RGB" in model["path"])
    depth_count = sum(1 for model in comb if "DEPTH" in model["path"])
    if rgb_count == depth_count:
        filtered_combinations.append(comb)


print("NUM COMB: {}".format(len(filtered_combinations)))

# Evaluate all the combinations
eval = []
for i, comb in enumerate(filtered_combinations):
    print("###########")
    print(f"Combination {i + 1}:")
    """
    rgb_models = [model["score"] for model in comb[0]]
    depth_models = [model["score"] for model in comb[1]]

    rgb_models_names = [model["path"] for model in comb[0]]
    depth_models_names = [model["path"] for model in comb[1]]
    """
    
    rgb_models = [model["score"] for model in comb if "RGB" in model["path"]]
    depth_models = [model["score"] for model in comb if "DEPTH" in model["path"]]

    rgb_models_names = [model["path"] for model in comb if "RGB" in model["path"]]
    depth_models_names = [model["path"] for model in comb if "DEPTH" in model["path"]]

    print(rgb_models_names)
    print("\n")
    print(depth_models_names)
    print("###########")
    res = evaluate_combination(rgb_models, depth_models, rgb_models_names, depth_models_names, combinations_lengths)
    eval.append(res)
